...
import requests, random, string, uuid, json, asyncio, re
...
def generate_random_device_id(length=16):
    return ''.join(random.choice('0123456789abcdef') for _ in range(length))

# Background task
async def background_task(num, limit):
    response_data = generate_uuid_and_sign_in(num, limit)
    print(response_data)

def is_valid_mobile_number(num):
    return re.match(r'^\d{10}$', num) is not None

@app.route('/api/user/<num>/<int:limit>', methods=['GET'])

async def process_request(num, limit):
    if limit <= 0:
    ...
    asyncio.create_task(background_task(num, limit))
    return jsonify({"status": "Bomber Active."}), 200
